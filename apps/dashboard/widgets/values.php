...
openDB();
	...
	echo "	<head>\n"; 
	 ...
	 echo "		<title>" . translate("ProposalsByCapex") . "</title>\n";
	...
	echo addCode("../assets/css/values.css");
	...
	echo "								left: 16,\n"; 
		...
		if (isset($_GET["d"])) {
			...
			echo "								data: [";
			$u = '';
			if ($t[0] != '') { $u .= "'" . translate("NotPresented") . ": R$ " . number_format($t[0]/100, 0, ',', '.') . "', "; }
			if ($t[1] != '') { $u .= "'" . translate("Waiting") . ": R$ " . number_format($t[1]/100, 0, ',', '.') . "', "; }
			if ($t[2] != '') { $u .= "'" . translate("Acepted") . ": R$ " . number_format($t[2]/100, 0, ',', '.') . "', "; }
			if ($t[3] != '') { $u .= "'" . translate("Recused") . ": R$ " . number_format($t[3]/100, 0, ',', '.') . "', "; }
	...
	echo "								},\n"; 
		...
		if (isset($_GET["d"])) {
			...
			echo "								data: [";
			$u = '';		
			if ($t[0] != '') { $u .= "{ value: " . $t[0] . ", name: '" . translate("NotPresented") . ": R$ " . number_format($t[0]/100, 0, ',', '.') . "' }, "; }
			if ($t[1] != '') { $u .= "{ value: " . $t[1] . ", name: '" . translate("Waiting") . ": R$ " . number_format($t[1]/100, 0, ',', '.') . "' }, "; }
			if ($t[2] != '') { $u .= "{ value: " . $t[2] . ", name: '" . translate("Acepted") . ": R$ " . number_format($t[2]/100, 0, ',', '.') . "' }, "; }
			if ($t[3] != '') { $u .= "{ value: " . $t[3] . ", name: '" . translate("Recused") . ": R$ " . number_format($t[3]/100, 0, ',', '.') . "' }, "; }
			if ($u != '') { echo left($u, strlen($u) - 2); }
			echo "]\n";
	...
	echo "		<div id='pie_donut'></div>\n"; 
		...
		echo "		<div class='pie-donut-title'><h4>" . translate("ProposalsStatus") . " - " . $s . "</h4><small>" . translate("ValueByCapex") . "</small></div>\n";
