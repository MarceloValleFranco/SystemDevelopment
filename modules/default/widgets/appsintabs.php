...
// APPSINTABS.PHP 1.0 (2019/01/09)

	// EACH GROUP OF APPS
	$d = select("*", "sysappsgroups", "where (CompanyID='" . COMPANYID . "' and ShowOnDesktop='1' and ItemActive<>'0') order by ItemOrder, ItemName");
	if ($d) {
		...
		echo "									<h5 class='card-title'>" . translate("SystemApplications") . "</h5>\n"; 
		...
		for ($x = 1; $x <= count($d); $x++) {
			...
			if ($e = select("*", "sysapps", "where (CompanyID='" . COMPANYID . "' and AppGroupID='" . $g[$x] . "' and ShowOnDesktop='1' and AppActive<>'0') order by ItemOrder, ItemName")) {
